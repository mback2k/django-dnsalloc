...
class JSONRPCServerMiddleware(object):
    ...
    def __init__(self):
        ...
        self.jsonrpc_urlpatterns = getattr(urls_module, urlpatterns_name, [])
        for application in django.conf.settings.INSTALLED_APPS:
            try:
                urls_mod_name = '%s.%s' % (application, django.conf.settings.ROOT_URLCONF)
                urls_module = _import_module(urls_mod_name)
                self.jsonrpc_urlpatterns += getattr(urls_module, urlpatterns_name, [])
            except ImportError:
                pass
